...
/// <returns>char*</returns>
char*	ToUppers(char *buf, const char *s) {
    char        *ptr = (char*) NULL;
    int         j = 0;
...
/// <param name="s">输入</param>
/// <returns>char*</returns>
char*	ToLower(char *buf, const char *s) {
    char        *ptr = (char*) NULL;
    int         j = 0;
...
/// <returns>char*</returns>
char*	Ldtoa(char *buf, LONG_DOUBLE num, int scale) {
	if (NULL == buf ) {
        return (char*) NULL;
    }
...
/// <param name="scale"></param>
/// <returns>char*</returns>
char*	FixedLdtoa(char *buf, LONG_DOUBLE num, int precision, int scale) {
	if (NULL == buf ) {
        return (char*) NULL;
    }
...
LONG_DOUBLE	Atold(const char *str) {
...
// 解析BOOL型字符串
int	ParseBoolString(const char *s) {
    char    *ptr = (char*) s;
    char    buf[8] = { 0 };
    int     value = 0;
...
// 若字符串为空则返回指定的值
const char*	SwitchEmptyString(const char *s, const char *v) {
    return IsEmptyString(s) ? v : s;
}

// 校验字符串是否是有效的数值型字符串
int	StringIsNumberType(const char *Buffer,const int Length,const int Decimal) {
    int i = 0;
    int len = Length;
    int dec = Decimal;
...
// 拷贝并返回字符串
char*	StrCopy(char *buf, const char *str, int length) {
...
// 连接并返回字符串
char*	StrCatT(char *buf, const char *str, const char *s2) {
	if (NULL == buf || NULL == str) {
		return (char*)NULL;
	}
...
int 	StrNoCaseCmp(char const *str1, char const *str2)
{
    int c1 = 0, c2 = 0;
    while(1)
    {
