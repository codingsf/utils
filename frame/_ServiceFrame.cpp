...
int	_ServiceFrame::RunService( int argc, char** argv )
{
	...
	char* lastslot = strrchr( exec_name, '/' );
	if( lastslot == NULL )
	{
		_exefilename = argv[0];
		_exefilelocation = "";
	...
	{
		_exefilename = &lastslot[1];
		lastslot[1] = 0;
		_exefilelocation = exec_name;
	}

	TRACE( "EXEFILENAME is <%s>", _exefilename.c_str() );
	TRACE( "EXEFILELOCATION is <%s>", _exefilelocation.c_str() );
...
{
	TRACE( "<%s> is running in DEBUG mode.", _exefilename.c_str() );
	char	cmd[ 1024 ] = {0};
	while( 1 )
	{
		fputs( ">", stdout );
