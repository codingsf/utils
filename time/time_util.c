...
BOOL
IsInTimeRange(const int TimeBegin, const int TimeEnd, const int JudgeTime)
{
	if (TimeBegin <= TimeEnd)
	{
		return (JudgeTime >= TimeBegin && JudgeTime <= TimeEnd);
	}
	
	const int JudgeEndTime = TimeEnd + 240000;
	return (JudgeTime >= TimeBegin && JudgeTime <= JudgeEndTime);
}
